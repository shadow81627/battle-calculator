...
<template>
  <table class="border-2 border-solid dark:border-slate-600" cellspacing="0" cellpadding="0">
    <thead>
      <tr class="dark:bg-slate-800 bg-[#d0d1d3] !print:bg-[#d0d1d3]">
        <th class="w-[2.2em] hidden sm:table-cell">
        ...
        <th class="px-2 text-left hidden sm:table-cell">
          Weapon
        </th>
        <th class="px-2">
          Range
    ...
    <tbody>
      <template v-for="(weapon, index) of  unit.weapons " :key="weapon.name">
        <template v-if="!weapon.profiles">
          <tr>
            <td :class="cellClasses(index)" width="100%" class="border-none sm:hidden table-cell text-center" colspan="8">
              <strong>{{ weapon.name }}</strong>
            </td>
          </tr>
          <tr>
            <td class="hidden sm:table-cell"></td>
            <td :class="cellClasses(index)" width="50%" class="hidden sm:table-cell">
              {{ weapon.name }}
              <template v-if="weapon.modifiers && weapon.modifiers.length">
                [<span v-for="(modifier, index) of  weapon.modifiers "
        ...
        <template v-else>
          <template v-for="( profile, index ) of  weapon.profiles " :key="profile.name">
            <tr>
              <td :class="profileClasses(index, weapon.profiles.length)" width="100%"
                class="border-none sm:hidden table-cell text-center" colspan="8">
            ...
            <tr>
              ...
              <td class="px-2 hidden sm:table-cell" width="50%" :class="profileClasses(index, weapon.profiles.length)">
                {{ profile.name }}
              </td>
              <td class="px-2 text-center" :class="profileClasses(index, weapon.profiles.length)">
                {{ profile.range }}
              </td>
              <td class="px-2 text-center" :class="profileClasses(index, weapon.profiles.length)">
                {{ profile.attack }}
