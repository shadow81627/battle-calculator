...
<template>
  <div class="container">
    ...
    >
      <div
        v-for="[key, value] of Object.entries(pickBy(_additions, Boolean)).sort(
          sortAdditions,
        )"
        :key="`${key}-${value}`"
      ...
      >
        <div
          class="flex flex-grow-1 items-center justify-center text-center capitalize"
        >
          {{ labels[key] ?? key }}
        </div>
    ...
    <div v-else class="row">
      <div class="col col-2 pr-5">
        ...
        <div class="text-center text-4xl font-700 font-barlow">&nbsp;</div>
      </div>
    </div>
  </div>
</template>
