...
function resultSum(sum: typeof defaultResult, result: typeof defaultResult) {
  for (const [key, value] of Object.entries(result)) {
    ...
    const isResultKey = (key: any): key is keyof typeof defaultResult =>
      key in defaultResult;
    if (isResultKey(_key) && typeof value === "number") sum[_key] += value;
  }
  return sum;
}
...
export default function averageWeaponCombat(data: {
  ...
  const rolls = data.rolls?.length
    ? data.rolls
    : [...Array(data.numberOfRolls)].map(() =>
        weaponCombat(data.weapon, data.unit, data.target, data.additional),
      );
  const sums = Object.entries(rolls.reduce(resultSum, { ...defaultResult }));
  const averages = Object.fromEntries(
    sums.map(([sumKey, sumValue]) => [
      sumKey.replace("random", "average"),
      sumValue / rolls.length,
    ]),
  );
