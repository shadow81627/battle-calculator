...
export default function weaponCombat(
  weapon: Weapon,
  unit: Unit,
  target: Unit,
  ...
  const attack = (() => {
    ...
    if (
      (additional?.order === "WAAAGH!" && weapon.range === "Melee") ||
      getModifier("Hierloom", weapon.modifiers)
    )
  ...
  const rapidFire = (() => {
    ...
    if (!modifier || !inRapidFireRange) return 0;
    return Number(modifier.name.match(/\d+/)?.[0]) ?? 0;
  })();
  const blast = getModifier("BLAST", weapon.modifiers)
    ? Math.floor(target.models / 5)
    : 0;
  const quantity = weapon.models || unit.models || 1;
  const randomAttacksTotalBonuses =
    (attack.base + blast + rapidFire) * quantity * minTurns;
  const randomAttackRolls = rolls(
    attack.rolls * quantity * minTurns,
    attack.rollType || 6,
  );
  const randomAttackRollsTotal = randomAttackRolls.reduce(
    (sum, roll) => sum + roll,
    0,
  );
  ...
  const _accuracy = (() => {
    const takeAim =
      (additional?.order === "take-aim" && weapon.range !== "Melee") ||
      (additional?.order === "fix-bayonets" && weapon.range === "Melee")
        ? 1
        : 0;
    ...
    if (
      target.abilities?.find((ability) => ability.name === "Mesmerising Form")
    )
      negatives.push(1);
    if (
      getModifier("HEAVY", weapon.modifiers) ||
      getModifier("Finely Balanced", weapon.modifiers)
    )
      buffs.push(1);
    ...
    if (
      ...
      (unit.abilities?.find((ability) => ability.name === "Wall of Dakka") &&
        inRapidFireRange) ||
      (unit.abilities?.find((ability) => ability.name === "Hogboss") &&
        weapon.range === "Melee")
    )
      buffs.push(1);
  ...
  const failedHitRolls = randomHitRolls.reduce(
    (sum, roll) => sum + (roll < _accuracy ? 1 : 0),
    0,
  );
  const randomHitReRolls = (() => {
    if (unit.abilities?.find((ability) => ability.name === "Daring Recon"))
      return rolls(occurrences(randomHitRolls)[1]);
    const hasFullReRolls = unit.abilities?.find((ability) =>
      ["Tank Hunter", "Flak Battery", "Targeting Array"].includes(ability.name),
    );
    if (hasFullReRolls && failedHitRolls > 0) return rolls(failedHitRolls);
    return [];
  })();
  const criticalHitRollsTotal = (() => {
    const crit =
      additional?.stratagem === "UNBRIDLED CARNAGE" && weapon.range === "Melee"
        ? 5
        : 6;
    const criticalWoundRollsEntries = Object.entries(
      occurrences([...randomHitRolls, ...randomHitReRolls]),
    );
    return criticalWoundRollsEntries.reduce(
      (sum, [key, value]) => (Number(key) >= crit ? sum + value : sum),
      0,
    );
  })();
  const sustainedHits = (() => {
    ...
    if (parsed.rolls)
      return (
        rolls(parsed.rolls, parsed.rollType).reduce(
          (sum, roll) => sum + roll,
          0,
        ) + parsed.base
      );
  ...
  const sustainedHitsTotal = sustainedHits
    ? sustainedHits * criticalHitRollsTotal
    : 0;
  const hasTorrent = getModifier("TORRENT", weapon.modifiers);
  const randomHitTotal = (() => {
    if (getModifier("TORRENT", weapon.modifiers)) return randomAttacksTotal;
    const rolled = [...randomHitRolls, ...randomHitReRolls];
    return (
      rolled.reduce((sum, roll) => sum + (roll >= _accuracy ? 1 : 0), 0) +
      sustainedHitsTotal
    );
  })();
  ...
  // Wounds
  const hasLethalHits =
    (getModifier("LETHAL HITS", weapon.modifiers) ||
  ...
  const strength = (function () {
    ...
    if (additional?.order === "WAAAGH!" && additional?.range === 1)
      buffs.push(1);
    if (
      unit.enhancements?.find(
        (enhancement) => enhancement.name === "The Everstave",
      ) &&
      weapon.range !== "Melee"
    )
      buffs.push(2);
    if (
      unit?.abilities?.find(
        (ability) => ability.name === "Daemon Lord of Tzeentch (Aura)",
      )
    )
      buffs.push(1);
    return (
      buffs.reduce((sum, value) => sum + value, 0) + Number(weapon.strength)
    );
  })();
  ...
  const anti = (() => {
    ...
    if (
      unit.abilities?.find((ability) => ["Titans' Bane"].includes(ability.name))
    )
      weaponModifiers.push(
        { name: "ANTI-MONSTER 4+" },
        { name: "ANTI-VEHICLE 4+" },
      );
  ...
  const failedWoundRolls = randomWoundRolls.reduce(
    (sum, roll) => sum + (roll < _wound ? 1 : 0),
    0,
  );

  const hasTankKiller =
    unit.abilities?.find((ability) =>
      ["Tank-killer", "Mobile Hunter-killers"].includes(ability.name),
    ) && targetIsVehicleOrMonster(target);
  const hasTwinLinked = getModifier("TWIN-LINKED", weapon.modifiers);
  const hasWoundReRolls =
    hasTwinLinked ||
    hasTankKiller ||
    unit.abilities?.find((ability) =>
      [
  ...
  const criticalWoundRolls = (() => {
    if (!anti) return 0;
    const criticalWoundRollsEntries = Object.entries(
      occurrences(randomWoundRolls),
    );
    return criticalWoundRollsEntries.reduce(
      (sum, [key, value]) => (Number(key) >= anti ? sum + value : sum),
      0,
    );
  })();
  const randomWoundTotal = (() => {
    const rolled = [...randomWoundReRolls];
    if (!anti || anti > _wound) rolled.push(...randomWoundRolls);
    const rolledTotal = rolled.reduce(
      (sum, roll) => sum + (roll >= _wound ? 1 : 0),
      0,
    );
  ...
  const hasDevastatingWounds = (() => {
    ...
    const ability = unit.abilities?.find(
      (ability) =>
        (["Mow Down the Enemy"].includes(ability.name) &&
          !targetIsVehicleOrMonster(target)) ||
        (["Titans' Bane"].includes(ability.name) &&
          targetIsVehicleOrMonster(target)),
    );
  ...
  const _piercing =
    weapon.piercing +
    (unit.abilities?.find((ability) => ability.name === "Point-blank Barrage")
      ? 1
      : 0) -
    (target.abilities?.find(
      (ability) => ability.name === "Ramshackle but Rugged",
    )
      ? 1
      : 0);

  const save = Math.min(target.attributes.save + _piercing, invulnerable);
  const randomSaveTotal =
    randomSaveRolls.reduce((sum, roll) => sum + (roll < save ? 1 : 0), 0) +
    randomDevastatingWounds;

  const _damage = parseRolls(weapon.damage);
  const randomDamageRolls = rolls(
    randomSaveTotal * _damage.rolls,
    _damage.rollType,
  );
  const randomDamageTotal = (() => {
    const hasHalfDamage = target?.abilities?.find(
      (ability) => ability.name === "Necrodermis",
    )
      ? 2
      : 1;
    const reduction = target?.abilities?.find(
      (ability) => ability.name === "Wall of Muscle",
    )
      ? 1
      : 0;
    const total = randomDamageRolls.reduce((sum, roll) => sum + roll, 0);
    if (_damage.rolls && !total) return 0;
    const flatDamage =
      _damage.base + (getModifier("Master-Worked", weapon.modifiers) ? 1 : 0);
    if (!_damage.rolls)
      return Math.max(
        1,
        Math.round((randomSaveTotal * flatDamage) / hasHalfDamage) - reduction,
      );
    return Math.max(
      1,
      Math.round((total + flatDamage) / hasHalfDamage) - reduction,
    );
  })();
  ...
  const randomPainTotal =
    randomDamageTotal -
    (pain
      ? randomPainRolls.reduce((sum, roll) => sum + (roll >= pain ? 1 : 0), 0)
      : 0);

  const casualties = (() => {
    let _casualties = 0;
    let currentModelWoundsRemaining = target.attributes.wound;
    const damageValues = randomDamageRolls.length
      ? randomDamageRolls
      : [...Array(randomSaveTotal)].map(() => 0);
    for (const roll of damageValues) {
      ...
      const damageTaken =
        damage -
        (pain
          ? painRolls.reduce((sum, roll) => sum + (roll >= pain ? 1 : 0), 0)
          : 0);
