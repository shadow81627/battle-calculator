  ...
  try {
    ...
    roster.costLimits?.costLimit.forEach((cl) => {
      ...
      if (cost && cl.value !== -1 && cost.value > cl.value) {
        errors[""] = errors[""] || [];
        errors[""].push(
          `Roster has ${cost.value}${cost.name}, more than the limit of ${cl.value}${cl.name}`,
        );
      }
    });
    ...
    roster.forces?.force.forEach((force, index) => {
      arrayMerge(
        errors,
        validateForce(roster, `forces.force.${index}`, force, gameData),
      );
    });
  } catch (e) {
    e.location = "roster";
...
const validateForce = (roster, path, force, gameData) => {
  ...
  try {
    gatherCatalogues(gameData.catalogues[force.catalogueId], gameData).forEach(
      (catalogue) => {
    ...
    force.selections?.selection.forEach((selection, index) => {
      arrayMerge(
        ...
        validateSelection(
          roster,
          `${path}.selections.selection.${index}`,
          selection,
          gameData,
        ),
      );
...
const validateSelection = (roster, path, selection, gameData) => {
  ...
  if (!entry) {
    arrayMerge(errors, {
      [path]: [
        `${selection.name} does not exist in the game data. It may have been removed in a data update.`,
      ],
    });
  ...
  try {
    ...
    const handleSelection = (selectionEntry) => {
      arrayMerge(
        ...
        checkConstraints(
          roster,
          `${path}.selections.selection.100000`,
          selectionEntry,
          gameData,
          true,
        ),
    ...
    const handleGroup = (group) => {
      if (group.hidden) {
        if (
          selection.selections?.selection.some(
...
const hasCategory = (subject, categoryId) => {
  return !!subject.categories?.category.some((c) =>
    c.entryId.includes(categoryId),
  );
};
...
export const countBy = (subject, entryId, entry, groupIds) => {
  ...
  let count =
    subject.forces?.force.filter((force) => force.entryId === entryId).length ??
    0;

  if (subject.forces) {
    if (entry.includeChildForces || entry.shared) {
      count += _.sum(
        subject.forces.force.map((force) =>
          countBy(force, entryId, entry, groupIds),
        ),
      );
    }
  }
  ...
  if (subject.selections) {
    if (entry.includeChildSelections || entry.shared) {
      count += _.sum(
        subject.selections.selection.map((selection) =>
          countBy(selection, entryId, entry, groupIds),
        ),
      );
    } else {
      count += _.sum(
...
const countByCategory = (subject, category, entry) => {
  ...
  let count =
    (!subject.catalogueId &&
      subject.categories?.category.some((c) => c.entryId === categoryId)) ??
    0;

  if (entry.includeChildSelections && subject.selections) {
    count += _.sum(
      subject.selections.selection.map((selection) =>
        countByCategory(selection, category, entry),
      ),
    );
  } else {
    count +=
      subject.selections?.selection.filter((selection) => {
        return selection.categories?.category.some(
          (c) => c.entryId === categoryId,
        );
      }).length ?? 0;
  }
  ...
  if (subject.forces) {
    if (entry.includeChildForces || entry.shared) {
      count += _.sum(
        subject.forces.force.map((force) =>
          countByCategory(force, category, entry),
        ),
      );
    }
  }
...
const sumCost = (subject, entry, filterByCategory) => {
  ...
  if (!subject.gameSystemId) {
    if (
      ...
      subject.categories?.category.some((c) => c.entryId === filterByCategory)
    ) {
      sum +=
        subject.costs?.cost.find((c) => c.typeId === entry.field)?.value | 0;
    }
  }

  if (
    (entry.includeChildForces || filterByCategory === "any") &&
    subject.forces
  ) {
    sum +=
      subject.forces.force.map((force) =>
        sumCost(force, entry, filterByCategory),
      ) | 0;
  }

  if (
    (entry.includeChildSelections || filterByCategory === "any") &&
    subject.selections
  ) {
    sum +=
      subject.selections.selection.map((selection) =>
        sumCost(selection, entry, filterByCategory),
      ) | 0;
  }
  ...
  try {
    ...
    if (entry.constraints) {
      entry.constraints?.forEach((constraint) => {
        ...
        const value = getConstraintValue(
          constraint,
          entry.id,
          subject,
          gameData,
        ...
        if (
          ...
          if (value === 1) {
            // errors.push(
            //   `${name} must have ${an(pluralize.singular(entryName))} selection`,
            // );
          } else {
            errors.push(
        ...
        if (
          constraint.type === "max" &&
          value !== -1 &&
          occurances > value * (subject?.number ?? 1)
        ) {
          if (value === 0) {
            // errors.push(
            //   `${name} cannot have ${an(pluralize.singular(entryName))} selection`,
            // );
          } else {
            errors.push(
              `${name} has ${occurances - value} too many ${entryName} selections`,
            );
          }
        }
        ...
        if (constraint.type === "exactly" && value !== occurances) {
          errors.push(
            `${name} must have ${value} ${entryName}, but has ${occurances}`,
          );
        }
      });
      ...
      if (max === 0) {
        errors = errors.filter(
          (e) => !e.match(/must have .* more|must have .* selection/),
        );
      }
    }
  } catch (e) {
...
const stringIncrement = (string, value) =>
  string.replace(numberRegex, (match) => parseFloat(match) + value);
...
const applyModifiers = (roster, path, entry, gameData, catalogue) => {
  ...
  function index(x, path = "") {
    if (typeof x == "object") {
      ...
      for (const attr of modifierAttributes) {
        x[attr]?.forEach((item, i) =>
          index(item, `${path ? path + "." : ""}${attr}.${i}`),
        );
      }
    }
  }
  ...
  const applyModifier = (modifier) => {
    ...
    const target =
      settable[modifier.field] || entry[modifier.field] !== undefined
        ? modifier.field
        : `${ids[modifier.field]}`;
    ...
    if (modifier.type === "set") {
      ...
      if (modifier.field !== "category") {
        debugger;
        throw new Error(
          "modifier.type === 'add' while modifier.field !== 'category'",
        );
      }
      ...
      if (modifier.field !== "category") {
        debugger;
        throw new Error(
          "modifier.type === 'remove' while modifier.field !== 'category'",
        );
      }

      entry.categoryLinks = entry.categoryLinks.filter(
        (link) => link.targetId !== modifier.value,
      );
    } else if (
      ...
      if (modifier.field !== "category") {
        debugger;
        throw new Error(
          `modifier.type === ${modifier.type} while modifier.field !== 'category'`,
        );
      }
      const category = entry.categoryLinks?.find(
        (cat) => cat.targetId === modifier.value,
      );
      if (category) {
...
const checkConditions = (roster, path, entry, gameData) => {
  const results = [
    ...(entry.conditions?.map((c) =>
      checkCondition(roster, path, c, gameData),
    ) || []),
    ...(entry.conditionGroups?.map((cg) =>
      checkConditions(roster, path, cg, gameData),
    ) || []),
  ];
...
export const pathParent = (path) =>
  path.replace(/.selections.selection.\d+$/, "");
...
const getSubject = (roster, path, condition) => {
  switch (condition.scope) {
    ...
    case "primary-catalogue":
      return {
        entryId: _.get(roster, pathToForce(path) || "forces.force.0")
          ?.catalogueId,
      };
    default: {
...
const getValue = (condition, subject, gameData, catalogue) => {
  const childId =
    findId(gameData, catalogue, condition.childId)?.targetId ||
    condition.childId;
  ...
  switch (constraint.field) {
    ...
    default: {
      let cost = constraint.field.startsWith("limit::")
        ? subject.costLimits?.costLimit.find(
            (cl) => `limit::${cl.id}` === constraint.field,
          ) ?? -1
...
const checkCondition = (roster, path, condition, gameData) => {
  ...
  switch (condition.type) {
    case "greaterThan":
      return (
        getValue(condition, subject, gameData, catalogue) > condition.value
      );
    case "lessThan":
      return (
        getValue(condition, subject, gameData, catalogue) < condition.value
    ...
    case "instanceOf":
      return []
        ...
        .some(
          (s) =>
            s.entryId?.endsWith(condition.childId) ||
            hasCategory(s, condition.childId),
        );
    case "notInstanceOf":
      return ![]
        .concat(subject)
...
let lastRoster;
let cache = {};
globalThis.cacheHits = 0;
globalThis.cacheMisses = 0;
...
export const getEntry = (roster, path, id, gameData, ignoreCache) => {
  ...
  if (roster !== lastRoster) {
    ...
    globalThis.cacheHits++;
    return cache[cachePath];
  }

  const catalogue = getCatalogue(roster, path, gameData);
  const entry = _.cloneDeep(
    findId(gameData, catalogue, _.last(id.split("::"))),
  );
  ...
  entry.rules?.forEach((rule, index) => {
    Object.defineProperty(entry.rules, index, {
      get: () =>
        getEntry(roster, path, `${baseId}::${rule.id}`, gameData, ignoreCache),
    });
  });

  cache[cachePath] = entry;
  globalThis.cacheMisses++;
