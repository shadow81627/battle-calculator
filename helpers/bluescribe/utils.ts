...
export const getCatalogue = (roster, path, gameData) => {
  const force = _.get(
    roster,
    path.replace(/forces.force.(\d+).*/, "forces.force.$1"),
  );
  if (!force) {
...
export const gatherCatalogues = (
  catalogue,
  gameData,
  catalogues = [gameData.gameSystem],
) => {
...
export const getMinCount = (entry) =>
  (!entry?.hidden &&
    entry?.constraints?.find((c) => c.type === "min" && c.scope === "parent")
      ?.value) ??
  0;
export const getMaxCount = (entry) =>
  entry.constraints?.find((c) => c.type === "max" && c.scope === "parent")
    ?.value ?? -1;
export const isCollective = (entry) =>
  ...
  (!entry?.hidden &&
    entry?.constraints?.find(
      (c) =>
        c.type === "min" &&
        (c.scope === "force" ||
          (c.scope === "roster" && countBy(roster, entry.id, entry) < c.value)),
    )?.value) ??
  0;
...
export const addForce = (roster, forceId, factionId, gameData) => {
  ...
  const force = {
    ...
    publications: {
      publication: [
        ...(gameData.catalogues[factionId].publications || []).map((p) =>
          _.pick(p, ["id", "name"]),
        ),
        ...(gameData.gameSystem.publications || []).map((p) =>
          _.pick(p, ["id", "name"]),
        ),
        ..._.flatten(
...
export const addSelection = (
  ...
  if (entryGroup) {
    if (getMaxCount(entryGroup) === 1) {
      base.selections.selection = base.selections.selection.filter(
        (s) => !s.entryGroupId?.endsWith(entryGroup.id),
      );
    }
  ...
  const handleGroup = (entryGroup) => {
    ...
    _.sortBy(
      entryGroup.selectionEntries,
      (e) => e.id !== entryGroup.defaultSelectionEntryId,
    ).forEach((selection) => {
...
export const refreshRoster = (roster, gameData) => {
  ...
  roster.forces.force.forEach((force, index) => {
    ...
    force.selections.selection.forEach((selection, selectionIndex) => {
      ...
      refreshSelection(
        newRoster,
        `forces.force.${index}.selections.selection.${selectionIndex}`,
        selection,
        gameData,
      );
    });
