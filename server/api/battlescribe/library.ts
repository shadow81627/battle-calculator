...
export default defineEventHandler(async () => {
  ...
  if (!data) throw createError({ message: "Unable to get file", status: 400 });
  ...
  function parseSelection(selectionEntry) {
    ...
    const categories = [];
    $(selectionEntry)
      .find("> categoryLinks > categoryLink")
      .each((index, categoryLink) => {
        categories.push({
          name: $(categoryLink).attr("name"),
          id: $(categoryLink).attr("targetId"),
        });
      });
    ...
    // const attributes = {};
    $(selectionEntry)
      .find("> profiles > profile")
      .each((index, profileElement) => {
        const profile = {
          name: $(profileElement).attr("name"),
          typeName: $(profileElement).attr("typeName"),
        };
        $(profileElement)
          .find("> characteristics > characteristic")
          .each((index, characteristicElement) => {
            // if ($(profileElement).attr('typeName') === 'Unit')
            //   profile[$(characteristicElement).attr('name')] = $(characteristicElement).text().match(/\d+/)?.[0] || 0
            // else
            profile[$(characteristicElement).attr("name")] = $(
    ...
    const constraints = [];
    $(selectionEntry)
      .find("> constraints > constraint")
      .each((index, constraintElement) => {
        const constraint = {
    ...
    $(selectionEntry)
      .find(
        '> selectionEntries > selectionEntry, > selectionEntry, > entryLinks > entryLink[type="selectionEntry"], > selectionEntry, > entryLinks > [type="selectionEntryGroup"], > selectionEntryGroups > selectionEntryGroup',
      )
      .each((index, childSelectionEntry) => {
        if ($(childSelectionEntry).attr("targetId")) {
          const targetElement = $(
            `#${$(childSelectionEntry).attr("targetId")}`,
          );
  ...
  $("sharedSelectionEntries > selectionEntry").each((index, selectionEntry) => {
    ...
    if (!selection.name?.includes("[Legends]")) selections.push(selection);
  });
